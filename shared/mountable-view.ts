...
type AbstractCtor<T = Record<string, never>> = abstract new (
  ...args: any[]
) => T;

export function MountableView<TBase extends AbstractCtor<ItemView>>(
  Base: TBase,
  prefix?: string,
) {
  abstract class Mountable extends Base {
    ...
    showLoading(
      ...
      const containerClass =
        classes?.container ?? `${this.#classPrefix}-loading-container`;
      const spinnerClass =
        classes?.spinner ?? `${this.#classPrefix}-loading-spinner`;
