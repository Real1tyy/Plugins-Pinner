...
export class SettingsUIBuilder<TSchema extends ZodObject<ZodRawShape>> {
  ...
  private setNestedValue(key: string, value: unknown): z.infer<TSchema> {
    const keys = key.split(".");
    const newSettings = JSON.parse(JSON.stringify(this.settings)) as Record<
      string,
      any
    >; // Deep clone
  ...
  private async updateSetting(key: string, value: unknown): Promise<void> {
    ...
    if (!result.success) {
      const errors = result.error.issues
        .map(
          (e) =>
            `${String(e.path.join("."))}${e.path.length > 0 ? ": " : ""}${e.message}`,
        )
        .join(", ");
  ...
  private inferSliderBounds(key: string): {
    min?: number;
    max?: number;
    step?: number;
  } {
  ...
  private inferArrayItemType(key: string): "string" | "number" | undefined {
    try {
      ...
      if (
        ...
        if (
          elementType &&
          (elementType as any)._def?.typeName === "ZodNumber"
        ) {
          return "number";
        }
        ...
        if (
          elementType &&
          (elementType as any)._def?.typeName === "ZodString"
        ) {
          return "string";
        }
      }
  ...
  addTextArray<T = string>(
    containerEl: HTMLElement,
    config: ArraySettingConfig<T>,
  ): void {
    const {
      key,
      name,
    ...
    const inferredItemType =
      config.itemType ?? this.inferArrayItemType(key) ?? "string";
    const parser =
      config.parser ??
      ((input: string) => {
