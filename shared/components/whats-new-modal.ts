...
export class WhatsNewModal extends Modal {
  ...
  async onOpen() {
    ...
    supportSection.createEl("h3", {
      text: "Support the development of this plugin",
    });
    ...
    toolsBtn.addEventListener("click", () => {
      window.open(
        this.config.links.tools ?? DEFAULT_WHATS_NEW_LINKS.TOOLS,
        "_blank",
      );
    });
    ...
    youtubeBtn.addEventListener("click", () => {
      window.open(
        this.config.links.youtube ?? DEFAULT_WHATS_NEW_LINKS.YOUTUBE,
        "_blank",
      );
    });
  }
