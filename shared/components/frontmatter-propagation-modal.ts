...
export class FrontmatterPropagationModal extends Modal {
  ...
  onOpen(): void {
    ...
    const changesContainer = contentEl.createDiv({
      cls: `${prefix}-frontmatter-changes`,
    });
    ...
    if (this.options.diff.modified.length > 0) {
      const modifiedSection = changesContainer.createDiv({
        cls: `${prefix}-changes-section`,
      });
    ...
    if (this.options.diff.deleted.length > 0) {
      const deletedSection = changesContainer.createDiv({
        cls: `${prefix}-changes-section`,
      });
