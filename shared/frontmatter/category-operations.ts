...
function transformCategoryProperty(
...
export function removeCategoryFromProperty(
  propertyValue: unknown,
  categoryName: string,
): unknown {
...
export async function deleteCategoryFromFile(
  ...
  await app.fileManager.processFrontMatter(file, (fm) => {
    if (fm[categoryProp]) {
      const updated = removeCategoryFromProperty(
        fm[categoryProp],
        categoryName,
      );
      if (
...
export async function renameCategoryInFile(
  ...
  await app.fileManager.processFrontMatter(file, (fm) => {
    if (fm[categoryProp]) {
      fm[categoryProp] = renameCategoryInProperty(
        fm[categoryProp],
        oldCategoryName,
        newCategoryName,
      );
...
export async function bulkDeleteCategoryFromFiles(
  ...
  return bulkCategoryOperation(
    files,
    (file) => deleteCategoryFromFile(app, file, categoryName, categoryProp),
    options,
  );
}
