...
export class Indexer {
  ...
  private async scanAllFiles(): Promise<void> {
    ...
    const relevantFiles = allFiles.filter((file) =>
      this.config.includeFile(file.path),
    );
  ...
  private buildFileSystemEvents$(): Observable<IndexerEvent> {
    ...
    const modified$ = this.fromVaultEvent("modify").pipe(
      this.toRelevantFiles(),
    );
    ...
    const renamedIntents$ = renamed$.pipe(
      ...
      filter(
        ([f]) => Indexer.isMarkdownFile(f) && this.config.includeFile(f.path),
      ),
      mergeMap(([f, oldPath]) => [
