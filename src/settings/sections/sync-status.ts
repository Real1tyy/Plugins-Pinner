...
export function renderSyncStatusSection(
	...
	// Status indicator
	const statusContainer = containerEl.createDiv({ cls: cls("status-container") });
	...
	if (localData.lastSyncAt) {
		...
		new Setting(containerEl).setName("Last Sync").setDesc(lastSyncDate.toLocaleString());
	}
	...
	if (downloadedEntries.length > 0) {
		...
		const downloadedContainer = containerEl.createDiv({ cls: cls("downloaded-list") });
		...
		for (const [pluginId, entry] of downloadedEntries) {
			...
			new Setting(downloadedContainer)
				.setName(pluginId)
				.setDesc(`Version: ${entry.version} | Downloaded: ${downloadDate.toLocaleDateString()}`);
		}
	}
	...
	if (localData.failedPlugins.length > 0) {
		...
		for (const failure of localData.failedPlugins) {
			...
			new Setting(failedContainer)
				.setName(failure.url)
				.setDesc(`Version: ${failure.version} | Error: ${failure.error} | Failed: ${failDate.toLocaleDateString()}`);
		}
	}
}
