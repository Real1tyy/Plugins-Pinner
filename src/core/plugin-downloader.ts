...
export class PluginDownloader {
	...
	async downloadPlugin(repoInfo: GitHubRepoInfo, version: string): Promise<DownloadResult> {
		try {
			...
			const manifestText = await this.githubService.downloadAssetAsText(assets.manifest);
			...
			const mainJsContent = await this.githubService.downloadAsset(assets.mainJs);
			await this.app.vault.adapter.writeBinary(`${pluginDir}/main.js`, mainJsContent);

			await this.app.vault.adapter.write(`${pluginDir}/manifest.json`, manifestText);

			let hasStyles = false;
			if (assets.styles) {
				try {
					const stylesContent = await this.githubService.downloadAssetAsText(assets.styles);
	...
	async isPluginInstalled(pluginId: string): Promise<boolean> {
		const pluginDir = this.getPluginPath(pluginId);
		const mainJsExists = await this.app.vault.adapter.exists(`${pluginDir}/main.js`);
		const manifestExists = await this.app.vault.adapter.exists(`${pluginDir}/manifest.json`);
