...
export class SyncManager {
	...
	constructor(
		...
		this.githubService = new GitHubService({ token: this.currentSettings.githubToken });
	...
	/**
	 * Runs the full sync process for all enabled plugins.
	 */
	async sync(): Promise<SyncSummary> {
		this._status = "syncing";
		this._lastError = null;

		const enabledPlugins = this.currentSettings.plugins.filter((p) => p.enabled);
		const results: PluginSyncResult[] = [];
		const newDownloaded: Record<string, DownloadedPluginEntry> = { ...this.localData.downloadedPlugins };
	...
	private async syncPlugin(plugin: PluginEntry): Promise<PluginSyncResult> {
		...
		if (downloadedEntry && this.versionsMatch(downloadedEntry.version, plugin.version)) {
			return {
				url: plugin.url,
				version: plugin.version,
				success: true,
		...
		// Download the plugin
		const result = await this.pluginDownloader.downloadPlugin(repoInfo, plugin.version);
	...
	private findDownloadedEntry(url: string): DownloadedPluginEntry | undefined {
		return Object.values(this.localData.downloadedPlugins).find((entry) => entry.url === url);
	}
