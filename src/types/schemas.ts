...
export const PluginsPinnerSettingsSchema = z
	.object({
		...
		autoSyncOnLoad: z.boolean().catch(SETTINGS_DEFAULTS.DEFAULT_AUTO_SYNC_ON_LOAD),
		showSyncNotification: z.boolean().catch(SETTINGS_DEFAULTS.DEFAULT_SHOW_SYNC_NOTIFICATION),
...
export const PluginsPinnerLocalDataSchema = z
	.object({
		downloadedPlugins: z.record(z.string(), DownloadedPluginEntrySchema).catch(LOCAL_DATA_DEFAULTS.DEFAULT_DOWNLOADED_PLUGINS),
		lastSyncAt: z.string().datetime().optional().catch(LOCAL_DATA_DEFAULTS.DEFAULT_LAST_SYNC_AT),
		failedPlugins: z.array(FailedPluginEntrySchema).catch(LOCAL_DATA_DEFAULTS.DEFAULT_FAILED_PLUGINS),
	})
	.strip();

export type PluginsPinnerLocalData = z.infer<typeof PluginsPinnerLocalDataSchema>;
